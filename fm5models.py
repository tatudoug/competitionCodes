...
for train, test in kf.split(x_data):
    ...
    if (True):
        ...
        fm.fit(X_train, y_train, n_more_iter=20)
        models.append(copy.deepcopy(fm)) # saves model
        ...
        print(100*np.sqrt(mean_squared_log_error(y_train,y_predT)))
        print(100*np.sqrt(mean_squared_log_error(y_test,y_pred )))
  ...
  """  
    ...
    # final prediction
fim = np.zeros(x_data_test.shape[0])
count = np.zeros(x_data_test.shape[0])
...
for k in np.arange(5):
    for num,p in enumerate(x_comp):
        try:
            finalPred = models[k].predict(p)
            if (finalPred) < 0:
                finalPred = 13
            
            fim[num] += np.round(finalPred)
            count[num] += 1           
            
        except:
            pass # if the number was not able to predict
    
    #fim[fim == -1] = 255 # sets the unpredicted values to the mean
    #fim = np.array(fim)
    #fim[fim < 0] = 13
    
df_test['num_orders'] = np.round(fim/5)
